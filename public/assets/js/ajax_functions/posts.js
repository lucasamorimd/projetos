...
function showPost(e) {
    ...
    a_comment.setAttribute("id", "likeCounts" + e.post.id_posts);
    a_comment.setAttribute("href", e.base + "/views/post_details.php?post=" + e.post.id_posts);
    a_comment.appendChild(i_comment);

    var span_like = document.createElement('span');
    $(span_like).attr("id", "likeCounts" + e.post.id_posts);
    span_like.innerHTML = 0 + "&#xB7;";

    var i_like = document.createElement('i');
    i_like.classList.add('material-icons');
    $(i_like).attr("id", "icon" + e.post.id_posts);
    ...
    $(a_like).attr("href", '#')
    $(a_like).attr("id", "like" + e.post.id_posts)
    ...
    span_nome.innerHTML = e.user_post.nome;
    ...
    img_avatar.setAttribute('src', e.base + '/avatar/' + e.user_post.avatar);

    var a_avatar = document.createElement('a');
    a_avatar.setAttribute('href', e.post.base + 'views/perfil.php?id=' + e.post.id_usuario);
    ...
    card_content.innerHTML = e.post.body;
    ...
    a_data.setAttribute('href', e.post.base + "/views/post_details.php?post=" + e.post.id_posts);
    a_data.innerHTML = e.post.data_criacao;
    ...
    $(input).attr("id", "id_post" + e.post.id_posts);
    $(input).attr("type", "hidden");
    $(input).attr("value", e.post.id_posts);
    $(input).attr("name", "id_post");

    var form = document.createElement('form');
    $(form).attr("id", "teste" + e.post.id_posts);
